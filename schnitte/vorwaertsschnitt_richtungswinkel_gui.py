...
class Anwendung(tk.Frame):
    ...
    def __init__(self, master=None):
        ...
        self.__eingabe_p3_nr: tk.Entry = tk.Entry(self)
        self.__eingabe_p3_y: tk.Entry = tk.Entry(self)
        self.__eingabe_p3_x: tk.Entry = tk.Entry(self)
        self.__eingabe_p4_nr: tk.Entry = tk.Entry(self)
        self.__eingabe_p4_y: tk.Entry = tk.Entry(self)
        self.__eingabe_p4_x: tk.Entry = tk.Entry(self)

        self.__eingabe_richtung_1n: tk.Entry = tk.Entry(self)
        self.__eingabe_richtung_13: tk.Entry = tk.Entry(self)
        self.__eingabe_richtung_2n: tk.Entry = tk.Entry(self)
        self.__eingabe_richtung_24: tk.Entry = tk.Entry(self)
        ...
        """
        pos = (1, 0)
        tk.Label(self, text="Punkt 3:").grid(row=pos[0], column=pos[1], sticky="ew")
        # Koordinaten laden
        if __name__ != "__main__":
            self.__eingabe_p3_nr.grid(row=pos[0], column=pos[1] + 1, sticky="ew")
            tk.Button(self,
                      text="Punkt 3 laden",
                      command=lambda: self.lade_punkt_3(self.__eingabe_p3_nr.get())) \
                .grid(row=pos[0], column=pos[1] + 2, sticky="w")
        # Koordinaten eingeben
        tk.Label(self, text="y 3:").grid(row=pos[0] + 1, column=pos[1])
        self.__eingabe_p3_y.grid(row=pos[0] + 1, column=pos[1] + 1, sticky="ew")
        tk.Label(self, text="[m]").grid(row=pos[0] + 1, column=pos[1] + 2, sticky="w")
        tk.Label(self, text="x 3:").grid(row=pos[0] + 2, column=pos[1])
        self.__eingabe_p3_x.grid(row=pos[0] + 2, column=pos[1] + 1, sticky="ew")
        ...
        # Eingabe: Richtung 1->3
        tk.Label(self, text="Richtung 1->3:").grid(row=pos[0] + 5, column=pos[1])
        self.__eingabe_richtung_13.grid(row=pos[0] + 5, column=pos[1] + 1, sticky="ew")
        ...
        """
        pos = (1, 3)
        tk.Label(self, text="Punkt 4:").grid(row=pos[0], column=pos[1], sticky="ew")
        # Koordinaten laden
        if __name__ != "__main__":
            self.__eingabe_p4_nr.grid(row=pos[0], column=pos[1] + 1, sticky="ew")
            tk.Button(self,
                      text="Punkt 4 laden",
                      command=lambda: self.lade_punkt_4(self.__eingabe_p4_nr.get())) \
                .grid(row=pos[0], column=pos[1] + 2, sticky="w")
        # Koordinaten eingeben
        tk.Label(self, text="y 4:").grid(row=pos[0] + 1, column=pos[1])
        self.__eingabe_p4_y.grid(row=pos[0] + 1, column=pos[1] + 1, sticky="ew")
        tk.Label(self, text="[m]").grid(row=pos[0] + 1, column=pos[1] + 2, sticky="w")
        tk.Label(self, text="x 4:").grid(row=pos[0] + 2, column=pos[1])
        self.__eingabe_p4_x.grid(row=pos[0] + 2, column=pos[1] + 1, sticky="ew")
        tk.Label(self, text="[m]").grid(row=pos[0] + 2, column=pos[1] + 2, sticky="w")
        tk.Label(self, text="").grid(row=pos[0] + 3, column=pos[1], columnspan=3)
        ...
        tk.Label(self, text="[gon]").grid(row=pos[0] + 4, column=pos[1] + 2, sticky="w")
        # Eingabe: Richtung 2->4
        tk.Label(self, text="Richtung 2->4:").grid(row=pos[0] + 5, column=pos[1])
        self.__eingabe_richtung_24.grid(row=pos[0] + 5, column=pos[1] + 1, sticky="ew")
        tk.Label(self, text="[gon]").grid(row=pos[0] + 5, column=pos[1] + 2, sticky="w")
    ...
    def berechnen(self):
        ...
        p3 = pkt.Punkt(gui.eingabefeld_auswerten(self.__eingabe_p3_y), gui.eingabefeld_auswerten(self.__eingabe_p3_x))
        p4 = pkt.Punkt(gui.eingabefeld_auswerten(self.__eingabe_p4_y), gui.eingabefeld_auswerten(self.__eingabe_p4_x))

        r_1n = gui.eingabefeld_auswerten(self.__eingabe_richtung_1n)
        r_13 = gui.eingabefeld_auswerten(self.__eingabe_richtung_13)
        r_2n = gui.eingabefeld_auswerten(self.__eingabe_richtung_2n)
        r_24 = gui.eingabefeld_auswerten(self.__eingabe_richtung_24)

        neupunkt = schnitte.vorwaertsschnitt.Vorwaertsschnitt.berechnen_richtungswinkel(p1, p2, p3, p4, r_1n, r_13, r_2n, r_24)
    ...
    def testdaten_laden(self):
        ...
        :rtype: None
        """
        testdaten = [24681.92, 90831.87, 24877.72, 89251.09, 23231.58, 91422.92, 22526.65, 89150.52, 331.6174, 0, 60.7510, 0]
        # Soll: y = 24052.79, Quelle: PGA Vorlesung
        ...
        gui.eingabefeld_schreiben(self.__eingabe_p3_y, testdaten[4])
        gui.eingabefeld_schreiben(self.__eingabe_p3_x, testdaten[5])
        gui.eingabefeld_schreiben(self.__eingabe_p4_y, testdaten[6])
        gui.eingabefeld_schreiben(self.__eingabe_p4_x, testdaten[7])
        gui.eingabefeld_schreiben(self.__eingabe_richtung_1n, testdaten[8])
        gui.eingabefeld_schreiben(self.__eingabe_richtung_13, testdaten[9])
        gui.eingabefeld_schreiben(self.__eingabe_richtung_2n, testdaten[10])
        gui.eingabefeld_schreiben(self.__eingabe_richtung_24, testdaten[11])
    ...
    def lade_punkt_3(self, p_pktnr):
        ...
        """
        p: pkt.Punkt = self.master.lade_punkt(p_pktnr)
        gui.eingabefeld_schreiben(self.__eingabe_p3_y, p.hole_y())
        gui.eingabefeld_schreiben(self.__eingabe_p3_x, p.hole_x())

    def lade_punkt_4(self, p_pktnr):
        """Lädt einen Punkt aus der Main-Punktliste und fügt ihn in die Eingabefelder ein.
        (erste_grundaufgabe_gui -> berechnungsfenster -> main -> arbeitsbereich -> tabelle)
        ...
        """
        p: pkt.Punkt = self.master.lade_punkt(p_pktnr)
        gui.eingabefeld_schreiben(self.__eingabe_p4_y, p.hole_y())
        gui.eingabefeld_schreiben(self.__eingabe_p4_x, p.hole_x())
    ...
    def json_laden(self) -> None:
        ...
        gui.eingabefeld_schreiben(self.__eingabe_p3_nr, json_daten["Punkte"]["P3"]["_Punkt__nr"])
        gui.eingabefeld_schreiben(self.__eingabe_p3_y, json_daten["Punkte"]["P3"]["_Punkt__y"])
        gui.eingabefeld_schreiben(self.__eingabe_p3_x, json_daten["Punkte"]["P3"]["_Punkt__x"])
        gui.eingabefeld_schreiben(self.__eingabe_p4_nr, json_daten["Punkte"]["P4"]["_Punkt__nr"])
        gui.eingabefeld_schreiben(self.__eingabe_p4_y, json_daten["Punkte"]["P4"]["_Punkt__y"])
        gui.eingabefeld_schreiben(self.__eingabe_p4_x, json_daten["Punkte"]["P4"]["_Punkt__x"])

        gui.eingabefeld_schreiben(self.__eingabe_richtung_1n, json_daten["Messung"]["R_1N"]["Richtung"])
        gui.eingabefeld_schreiben(self.__eingabe_richtung_13, json_daten["Messung"]["R_13"]["Richtung"])
        gui.eingabefeld_schreiben(self.__eingabe_richtung_2n, json_daten["Messung"]["R_2N"]["Richtung"])
        gui.eingabefeld_schreiben(self.__eingabe_richtung_24, json_daten["Messung"]["R_24"]["Richtung"])
